...
class ModalSandbox(Sandbox):
    ...
    def stop(self) -> None:
        ...
        # started, so a failure in between — `_start_driver`,
        # `create_context`, building `SandboxInfo` — would otherwise leave a
        # real, running sandbox that `stop()` skips entirely and this object
        # can never clean up again.
