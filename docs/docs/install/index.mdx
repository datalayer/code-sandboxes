...
Code Sandboxes supports different execution providers via extras:
...
- For Datalayer variant: `code-sandboxes[datalayer]` and `DATALAYER_API_KEY`,
  or a session from `datalayer login`
...
| `DATALAYER_RUNTIMES_URL` | Custom Datalayer runtimes URL (optional; defaults to `https://r1.datalayer.run`) |
...
| `E2B_DOMAIN` | E2B domain (optional, for a self-hosted cluster; defaults to e2b.app) |
