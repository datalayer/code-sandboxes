...
2. Export the URL, which is required, and the key, which every bridge
   deployed with one requires — a bridge on the public internet with no key is
   a sandbox anybody may run code in, and only a local development bridge
   should have none. The provider is listed as available only when both are
   set:
   ```bash
...
- Reading a variable is **refused**, with the reason: `get_variable` binds the
  value in one execution and reads it back in a second, and this provider's
  first process is gone before the second starts. `sandbox.commands.run` is
  built on it: the command still runs, but its output cannot be read back, so
  it answers with `exit_code=-1` and that reason in `stderr` rather than
  raising. Have the snippet print what you need, or keep it in a file.
