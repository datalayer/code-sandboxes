...
`marimo`, `modal`, and `monty`. Older `local-*` names are no longer supported.

A name is read in whatever case and separator it arrives in: `google-colab`,
`google_colab` and `GOOGLE_COLAB` all name the same variant. A space does not
work — `normalize_variant` folds underscores, not whitespace.
...
| [`marimo`](./marimo) | A Jupyter kernel holding Marimo's reactive cell graph: running a cell re-runs what depends on it |
...
What a variant cannot do is hold a namespace open when its provider gives it
...
def on_output(msg: OutputMessage):
    print(f"[{'stderr' if msg.error else 'stdout'}] {msg.line}")
...
```python
# Get sandbox ID for later — it exists only once the sandbox is started
sandbox = Sandbox.create(variant="datalayer")
sandbox.start()
...
```python
# List running Datalayer sandboxes — Sandbox.list() answers for that variant
for sandbox in Sandbox.list():
    print(f"{sandbox.sandbox_id}: {sandbox.info.status}")

# Any other variant answers through its manager, with SandboxInfo records
from code_sandboxes import get_manager

for info in get_manager("modal").list():
...
# Every variant stops the same way
sandbox.stop()

# terminate() and kill() are datalayer aliases for stop()
sandbox.terminate()
sandbox.kill()
