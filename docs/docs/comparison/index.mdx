...
Code Sandboxes provides a unified API across all supported variants (`eval`, `monty`, `docker`, `jupyter`, `kaggle`, `google-colab`, `modal`, `datalayer`), with native Jupyter kernel support.
