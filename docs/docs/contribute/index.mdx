...
- None for the core local suites (`eval`, factory, model).

Optional test variables:

- `RUN_JUPYTER_TESTS=1` runs the local Jupyter Server tests, which skip
  themselves without it — the one variable the suite reads.
...
Makefile targets, a page under `docs/docs/providers/`, and a test module. Two
...
- **What a provider cannot do is refused, not ignored.** A GPU asked of a
...
- [Guide](/guide) — what the package adds, and how the providers differ
