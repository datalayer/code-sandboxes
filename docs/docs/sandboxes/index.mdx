...
| Variant | Summary |
|---------|---------|
| `jupyter` | Jupyter kernel-backed execution with persistent state |
...
| `colab` | Google Colab runtime via runtime proxy |
...
For the complete authentication, channels URL, batch execution, and accelerator
reference, see [Kaggle](./kaggle).
...
kernel behind an authenticating proxy, using the `ColabKernelClient` implemented
by Code Sandboxes.

For the complete proxy authentication and channels URL guide, see
