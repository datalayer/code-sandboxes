...
Canonical variant names are `eval`, `docker`, `jupyter`, and `datalayer`.
Older `local-*` names are no longer supported.
...
Code Sandboxes provides two categories of execution backends:

### Local Sandboxes

Local sandboxes execute code in-process, sharing memory with the host Python process.
...
Remote sandboxes execute code out-of-process via the Jupyter kernel protocol, providing 
better isolation and persistent kernel state.
