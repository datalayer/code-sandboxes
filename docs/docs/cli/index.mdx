...
They are declared once, where the sandbox is made, because what is worth
running depends on what was created — a sandbox with an H100 in it wants
device discovery, one that runs in this very process wants neither:
...
- `coreweave`: starts a CoreWeave sandbox and holds a session process for it,
  so lines share a namespace. `--gpu` takes a CoreWeave GPU kind; CoreWeave
  places a sandbox on runners of one kind only, so if several are given
  comma-separated the first one is used rather than walked as a fallback list.
