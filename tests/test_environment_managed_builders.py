...
class TestTheHalfThatIsNotBuiltYet:
    @pytest.mark.parametrize("variant,item", [("e2b", "E2-03")])
    def test_a_build_refuses_by_naming_what_is_missing(self, variant: str, item: str) -> None:
        ...
        Daytona (E2-04) and Modal (E2-05) are both built now, each covered
        by its own test_environment_{daytona,modal}_builder.py, so neither
    ...
    @pytest.mark.parametrize("variant", ["e2b"])
