...
#: A realistic export: the author's own package, plus Datalayer's protected
#: pins at the versions this checkout's own constraints file requires — a
#: pyproject source is never merged with them (E3-01), but the lock brought
#: still has to agree that they are there (found by Copilot review, PR #27).
EXPORTED = (
...
class TestAPyprojectFile:
    ...
    def test_a_lock_missing_a_protected_pin_is_refused(self) -> None:
        """Never merged in for this source (E3-01) — but still required: the
        sandbox needs the kernel stack to connect, whatever brought the lock."""
        only_six = (
            "# This file was autogenerated by uv via the following command:\n"
            "#    uv export --locked --format requirements.txt\n"
            "six==1.16.0 \\\n    --hash=sha256:" + "aa" * 32 + "\n"
        ...
        assert raised.value.code.code == "DL_ENV_PROTECTED_PACKAGE"
        assert raised.value.detail["package"] == "ipykernel"

    def test_a_lock_that_contradicts_a_protected_pin_is_refused(self) -> None:
        wrong_version = (
            "# This file was autogenerated by uv via the following command:\n"
            "#    uv export --locked --format requirements.txt\n"
            "six==1.16.0 \\\n    --hash=sha256:" + "aa" * 32 + "\n"
        ...
        assert raised.value.code.code == "DL_ENV_PROTECTED_PACKAGE"
        assert raised.value.detail["package"] == "ipykernel"
        assert raised.value.detail["locked"] == "6.0.0"
...
class TestTheBuildkitRunner:
    ...
    def test_an_index_url_cannot_end_the_run_line_and_start_another_command(self) -> None:
        ...
        index is a spec field a user wrote, checked only for `https://` and
        no embedded credential — never for shell metacharacters."""
        import shlex

        from code_sandboxes.environments.resolve import BuildkitResolveRunner

        injected_path = "/tmp/pwned"  # noqa: S108 - the payload, never a real path this test touches
        malicious = f"https://example.com/simple; touch {injected_path} #"
        ...
        run_line = next(line for line in dockerfile.splitlines() if "uv pip compile" in line)
        # Shell-tokenizing the generated line the way `/bin/sh -c` would must
        # yield the URL back as one argument, `;` and `#` included — never a
        # second command.
        tokens = shlex.split(run_line.removeprefix("RUN "))
        assert malicious in tokens
        assert "touch" not in tokens and injected_path not in tokens
