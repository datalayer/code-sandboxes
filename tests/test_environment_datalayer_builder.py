...
class TestTheDockerfileItGenerates:
    ...
    def test_the_frontend_is_new_enough_for_the_mount_it_emits(self) -> None:
        """The `env=` mount and the `# syntax=` pin have to agree.

        `--mount=type=secret,env=NAME` does not exist before frontend 1.10:
        1.7, which this pinned for its first year, answers `unexpected key
        'env' in 'env=NAME'` and fails the whole build. Nothing caught it
        because no build had ever carried a build secret — the first one that
        did, on r1 on 2026-09-17, died on exactly that.

        Asserted against the rendered Dockerfile rather than the constant, so
        lowering the pin fails here whatever else is refactored.
        """
        from code_sandboxes.environments.adapters.datalayer import (
            SECRET_ENV_FRONTEND,
        )
        ...
        request = a_request(spec=A_SECRET, build_secret_ids=("dlsec_01J9BUILDSECRET0000000000",))
        dockerfile = a_builder().dockerfile(request)
        assert "env=PIP_TOKEN" in dockerfile, "this test needs an env-mounted secret"
        syntax = dockerfile.splitlines()[0]
        assert syntax.startswith("# syntax=docker/dockerfile:")
        pinned = tuple(int(part) for part in syntax.split(":")[-1].split(".")[:2])
        assert pinned >= SECRET_ENV_FRONTEND, (
            f"frontend {syntax} has no `env=` secret mount; it needs at least "
            f"{'.'.join(str(n) for n in SECRET_ENV_FRONTEND)}"
        )
